...
partitionSize <- round(nrow(data)/10) - 1
rowCount <- 1
...
accuracyVec <- vector()
for (i in 1:10) {
  upperBound <- min(rowCount + partitionSize, nrow(data))
  testData <- data[rowCount:upperBound,]
  trainingData <- data[-(rowCount:upperBound),]
  trainingData$revenue <- as.factor(trainingData$revenue)
  
tree <- rpart(revenue~budget+popularity+vote_count, data = trainingData, method = "class")

# build decision forest
forest <- randomForest(revenue~budget+popularity+vote_count, data = trainingData, importance = TRUE, ntree = 1000, na.action=na.roughfix)
# plots regression
...
table.traintree <- table(tree.train, testData$revenue)
table.trainforest <- table(forest.train, testData$revenue)
...
rf_default <- train(revenue~budget+popularity+vote_count, 
                      data=data, 
                      method='rf', 
                      metric='RMSE', 
                      tuneGrid=tunegrid, 
